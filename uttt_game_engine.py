...
def draw_board(uttt_board, next_board=None):
    ...
    #     r_jump = int(nextToPlayIn[0] * 5)
    #     c_jump = int(nextToPlayIn[1] * 8)
    #     # Top/bottom sides
    #     for row in [r_jump, r_jump + 5]:
    #         for col in [c_jump + 2, c_jump + 4, c_jump + 6]:
    #             board_matrix[int(row)][int(col)] = " "
    #     # Left/right sides
...
class MiniBoard:
    def __init__(self, other=None):
        ...
        if other is not None:
            ...
            self._legal_cells = [i for i in range(9) if self._board[i] == EMPTY]
            self._assess_board()
        else:
            self._board = [EMPTY] * 9
...
class UTTTBoard:
    def __init__(self, boards=list()):
        ...
        while i < 9:
            ...
            i += 1
        self._winner = None
        self._available_boards = [i for i in range(9) if
